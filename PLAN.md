...
Phase 4  ████████████████████████   7/7   (100%)  AI & Automation   ✅ DONE
─────────────────────────────────────────────────
TOTAL                               42/50  (84.0%)

Phase 4 extended (not in original plan):
  + CRUD completeness (get/update/delete) — RFC-005
  + Workflow routing (forgeplan route) — RFC-005
  + MCP config (.mcp.json) — RFC-005
  + 24 CLI integration tests (full workflow dogfood)
  = 20 CLI commands, 20 MCP tools, 176 tests
```
...
- [x] **4.1** MCP server mode — `forgeplan serve` (rmcp crate)
- [x] **4.2** LLM integration — generate PRD from description (`forgeplan generate`)
- [x] **4.3** FPF ADI cycle — `forgeplan reason` (Abduction→Deduction→Induction)
- [x] **4.4** Auto-decompose — `forgeplan decompose` (PRD → RFC tasks)
- [x] **4.5** Evidence Decay — `forgeplan decay` (R_eff impact report)
- [x] **4.6** Depth calibration — `forgeplan calibrate` (heuristic depth suggestion)
- [x] **4.7** Auto-capture — `forgeplan capture` (decision → Note/ADR via LLM)
