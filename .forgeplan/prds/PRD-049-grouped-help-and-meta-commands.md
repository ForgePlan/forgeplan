...
|----|-----------|--------|---------|--------|-----------|----------------|
| SC-1 | Grouped help visible | Набор категорий в выводе `forgeplan --help` | {} | {CORE, QUALITY, REASONING, LIFECYCLE, DISCOVERY, ESTIMATE, AUDIT, PRO} | v0.20.0 | Integration test parses `--help` output and asserts exact set of heading strings (not grep count — fragile across clap minor versions; snapshot test with `insta` crate) |
