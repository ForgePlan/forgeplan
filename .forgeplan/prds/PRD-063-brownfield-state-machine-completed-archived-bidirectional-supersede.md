...
# PRD-063: Brownfield — state machine completed archived + bidirectional supersede
...
- **AC-8**: Brownfield migration (PRD-059 migrate --apply) может ставить `completed`/`archived` напрямую из source `status: done`/`status: archived`.
...
- **FR-7** Status-map extension (PRD-059 brownfield migration): `status: done` → `completed`, `status: archived` → `archived`.
...
| PRD-059 | PRD | informs (migration ставит completed/archived напрямую) |
