...
- NOT содержит classification logic (отдельно в PRD-C skill)
...
- **FR-1** `forgeplan discover`: scan source dirs, parse frontmatter, collect: file path, detected kind candidates (heuristic), frontmatter schema, size, detected wikilinks count. Output — migration-plan.json по schema.
- **FR-2** Migration plan JSON schema: fields version, files с path, size, detected kind candidates, frontmatter, predicted kind, decision, mapped id, warnings. Schema файл — docs/schemas/migration-plan.schema.json.
- **FR-3** `forgeplan migrate --dry-run`: read plan, для каждого файла с decision/predicted kind показать destination в stdout. Без writes.
...
- **FR-8** scan-import становится thin wrapper: равно discover + migrate --apply --auto-classify --yes. Deprecation warning v0.25, removal v0.27.
