...
### Derived-skill-file policy (PROB-040 C2 amendment, 2026-04-21)

Skill-файлы устанавливаются installer'ом **вне** `.forgeplan/` (в `.claude/skills/`, `.cursor/skills/` и т.п.). ADR-003 invariant «markdown primary + LanceDB derived» scoped только на `.forgeplan/` — для установленных skill-файлов требуется отдельная policy, иначе update может затирать user edits:

- **Canonical source**: `marketplace/<pack>/skills/<name>/SKILL.md` в git-репо пакета. Это primary.
- **Installed copy** в `.claude/skills/<name>/SKILL.md` etc. — **derived**. Считается read-only от perspective forgeplan-core; user edits разрешены но не сохраняются при `skill update`.
- **Hash-tracking** (`.forgeplan/.skill-installs.json`): installer сохраняет hash каждого установленного файла at install time. При `skill update`:
  - Hash совпадает с installer record → update без prompt
  - Hash не совпадает (user edited) → блокирует update, требует `skill eject` (разрывает ownership — installer больше не tracks этот файл) или `--force` (перезапишет, user edits lost).
- **`forgeplan skill eject <name>`** — новая команда (добавляется в PRD-069 scope): удаляет entry из `.skill-installs.json`, файл остаётся на диске но становится user-owned. Future updates через installer этот файл trogать не будут.
- **Round-trip invariant**: canonical → installed, **never** installed → canonical. Если user хочет изменить skill globally — edits в pack source, не в installed copy.
...
3. Revert `init --from-brownfield` к noop. If `discover`/`migrate` disabled — restore original pre-wrapper scan-import implementation (wrapper из v0.25 inoperable без них)
...
| `crates/forgeplan-core/src/lifecycle/mod.rs` | — | Extended state machine (PRD-063) |
| `crates/forgeplan-core/src/artifact/kind.rs` | — | New kinds (PRD-064) |
...
### Phase 1: Core (PRD-059 + PRD-060 foundation)
...
### Phase 2: Cross-harness (PRD-061 + PRD-062)
...
### Phase 3: Extensions (PRD-063 + PRD-064)
...
- [ ] **4.4** `scan-import` removed в v1.0 (≥3 minor releases deprecation window)
...
| EPIC-006 | Epic | drives (to be created — Brownfield Migration Pipeline) |
| PRD-059 | PRD | drives (discover + migrate core) |
| PRD-060 | PRD | drives (self-description + agent-manifest + context injection) |
| PRD-061 | PRD | drives (marketplace brownfield-pack) |
