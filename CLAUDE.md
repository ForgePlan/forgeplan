...
### Правило контекста (1M window):

- **Максимум 2 спринта на сессию** — после 2-го спринта закрой сессию, сохрани в Hindsight, продолжи в новом чате
- **Перед каждым новым спринтом** — оцени объём сделанной работы. Если уже 50+ tool calls или 2+ PR → закрывай сессию
- **Длинная сессия = ошибки**: squash merge потери, забытые тесты, velocity bias — всё из-за усталости контекста
- **Новый чат = свежий старт**: `/recall` восстановит контекст, `forgeplan health` покажет состояние
