...
Тот же принцип действует для **напрямую слинкованной эвиденции** (ADR-020): пакеты с терминальным статусом (`superseded`/`deprecated`) исключаются из min — вытесненное показание остаётся видимым в breakdown, но не участвует в счёте. Draft-эвиденция здесь СЧИТАЕТСЯ (в отличие от draft-зависимостей): это свежее измерение, ждущее активации.

Пропущенные зависимости и эвиденция видны в `forgeplan score`: `"Skipped EPIC-002 (status: draft)"`, `"Skipped EVID-012 (status: superseded)"`.
...
Полный глоссарий всех терминов см. в [GLOSSARY.ru.md](GLOSSARY.ru.md).

Ключевые термины, которые нужно запомнить:

| Термин | Полное название | Что значит простыми словами |
|--------|----------------|---------------------------|
...
**Как правильно**: Починить слабое evidence. Или честно вытеснить: слинковать пакет ре-верификации и `supersede <старый> --by <новый>` — терминальный пакет уходит из min (ADR-020), оставаясь в графе как история.
