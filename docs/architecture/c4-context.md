...
The system is distributed as a single binary (brew, install.sh, cargo) and a stdio-transport MCP server that any MCP-compatible AI agent (Claude Code, Cursor, Aider, Continue) can connect to through 63 tools.
...
- MCP server (stdio transport, 63 tools)
...
- **Key Features Used**: All 63 MCP tools — most frequently: `forgeplan_route`, `forgeplan_new`, `forgeplan_validate`, `forgeplan_reason`, `forgeplan_score`, `forgeplan_activate`, `forgeplan_search`, `forgeplan_drift`, `forgeplan_dispatch`
...
- **Description**: Full MCP server over stdio transport exposing 63 tools to any MCP-compatible AI agent. Tools cover the complete methodology workflow — from routing through activation — plus multi-agent dispatch and identity stamping.
...
1. **Connect**: Forgeplan MCP server starts via stdio; Claude Code discovers 63 tools
...
C4Context
    ...
    Rel(ai_agent, forgeplan, "Calls methodology tools during coding sessions", "MCP stdio (63 tools)")
