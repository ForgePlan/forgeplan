...
|---|---|---|---|
| `data` | `string` | да | Экспорт JSON в виде строки. Передайте полное содержимое пакета, созданного с помощью `forgeplan_export`. Для больших пакетов предпочтительнее использовать CLI, который может читать из файла. |
| `force` | `bool` | нет (по умолчанию: `false`) | Перезаписывать артефакты, которые уже существуют по ID. Используйте с осторожностью. |
...
- [`forgeplan import <file>`](/docs/cli/import/) — читает непосредственно из файла, более эргономично для больших пакетов.

## См. также

- [Обзор MCP](/docs/mcp/)
- [`forgeplan_export`](/docs/mcp/forgeplan_export/) — создание пакета
- [`forgeplan_init`](/docs/mcp/forgeplan_init/) — создание рабочего пространства перед импортом
- [`forgeplan_list`](/docs/mcp/forgeplan_list/) — проверка импорта
