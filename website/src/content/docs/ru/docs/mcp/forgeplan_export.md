...
|---|---|---|---|
| `output` | `string` | нет | Путь к целевому файлу. Если опущено, инструмент возвращает JSON-пакет в виде встроенных данных в ответе. Если указано, инструмент записывает данные на диск и возвращает краткое сообщение об успешном выполнении. |
...
-   [`forgeplan export`](/docs/cli/export/) — идентичное поведение из терминала.

## См. также

-   [Обзор MCP](/docs/mcp/)
-   [`forgeplan_import`](/docs/mcp/forgeplan_import/) — восстановление из пакета
-   [`forgeplan_init`](/docs/mcp/forgeplan_init/) — инициализация рабочего пространства перед импортом
