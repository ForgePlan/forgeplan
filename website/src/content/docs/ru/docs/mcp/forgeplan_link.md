...
|---|---|---|---|
| `source` | `string` | да | ID исходного артефакта. |
| `target` | `string` | да | ID целевого артефакта. |
| `relation` | `string` | нет (по умолчанию: `"informs"`) | Тип отношения: `informs`, `based_on`, `supersedes`, `contradicts`, `refines`. |
...
- [`forgeplan link`](/docs/cli/link/) — та же операция, позиционные аргументы

## См. также

- [Обзор MCP](/docs/mcp/)
- [`forgeplan_score`](/docs/mcp/forgeplan_score/) — использует граф связей для R_eff
- [`forgeplan_graph`](/docs/mcp/forgeplan_graph/) — визуализирует граф связей
- [`forgeplan_supersede`](/docs/mcp/forgeplan_supersede/) — версия связи `supersedes` с сохранением состояния
