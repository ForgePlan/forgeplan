...
`forgeplan score` вычисляет **R_eff** (эффективную надёжность) артефакта-решения на основе связанных с ним **не-терминальных** EvidencePack — вытесненные пакеты (`superseded`/`deprecated`) остаются в списке с пометкой «excluded from min», но в оценку не входят. R_eff следует правилу «слабого звена» из Quint-code: `R_eff = min(evidence_scores)` - **никогда не среднее значение**. Одно слабое доказательство подрывает всё решение, и в этом весь смысл: PRD, подкреплённый одним надёжным бенчмарком и одним опровергнутым тестом, всё равно остаётся рискованным PRD.
...
Слабое звено (EVID-018) снижает R_eff. Либо усильте это доказательство, опровергните его, либо замените — замена означает слинковать новый пакет и выполнить `forgeplan supersede EVID-018 --by <НОВЫЙ-EVID>`: вытесненный пакет уходит из min, и оценка восстанавливается сразу.
