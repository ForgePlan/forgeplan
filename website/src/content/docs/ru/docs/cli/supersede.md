...
- **EvidencePack** опровергнут более поздней ре-верификацией (например, старый `refutes`-пакет после исправления дефекта): слинкуйте новый пакет, затем вытесните старый — вытесненный пакет уходит из R_eff min родительского артефакта, кэш оценки обновляется сразу.
