...
`forgeplan setup` делает две вещи, которые `cargo install` не может сделать сам:

- создаёт **алиас `fpl`** — brew и `install.sh` получают его из `bin-aliases`
  cargo-dist, но у cargo нет post-install хука, поэтому при сборке из исходников
  есть `forgeplan` и нет `fpl`
