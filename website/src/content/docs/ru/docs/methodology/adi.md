...
```mermaid
flowchart TD
  A["Абдукция\n3+ гипотезы"] --> B["Дедукция\nПрогнозы для каждой H"]
  B --> C["Индукция\nПроверка доказательствами"]
  C --> D{Лучшая гипотеза?}
  D -->|Явный лидер| E[Действовать уверенно]
  D -->|Компромиссы| F[Зафиксировать в ADR]
  D -->|Нет лидера| G[PoC или больше доказательств]
```
