...
```mermaid
flowchart TD
  V["Вердикт\nподтверждает=1.0 | ослабляет=0.5 | опровергает=0.0"] --> S["оценка = вердикт - штраф CL"]
  CL["Штраф CL\nCL3=0.0 | CL2=0.1 | CL1=0.4 | CL0=0.9"] --> S
  S --> R["R_eff = min(все оценки)\nСлабое звено, не среднее"]
```
...
## Какие пакеты участвуют в min

R_eff считается только по **текущей** эвиденции артефакта. Пакеты с терминальным статусом — `superseded` или `deprecated` — исключаются из min: вытесненный пакет остаётся в графе как история (и остаётся в выводе `forgeplan score` с пометкой «excluded from min»), но о текущей надёжности больше не говорит. Draft-эвиденция **считается** (это штатное состояние свежего измерения до активации), а **активный** `refutes`-пакет по-прежнему обнуляет оценку.

Честный способ убрать устаревший слабый пакет — вытеснение, а не удаление или правка вердикта: слинкуйте пакет ре-верификации и выполните `forgeplan supersede <старый> --by <новый>`. Если *вся* связанная эвиденция терминальна — артефакт деградирует к «no active evidence» (R_eff 0.0): для восстановления пакет-замена должен быть слинкован, а не просто существовать. Refutes/weakens-пакет, закрытый *без* наследника, помечается аномалией `unbacked_displacement`.
